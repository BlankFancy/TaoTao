...
if (isset($_POST['Submit'])) {
    ...
    if (empty($errors)) {
        require_once('./conn.php');
        $sql = "SELECT cake_id FROM cake WHERE name='{$_POST['name']}'";
        $result = mysqli_query($dbc,$sql);
        if (mysqli_num_rows($result) == 0) {

            $sql = "INSERT INTO `cake` ( `cake_id` , `name` , `price`  ) 
            ...
            if($result){
                ...
                echo '<b>添加成功！</b><a href=\'cakeView.php\'>查看</a>';
                exit();
            }else{
...
?>
    ...
    <h2>添加商品信息：</h2>
    <form action="cakeInsert.php" method="post">
